...
data = pd.DataFrame()
for root, dir, files in os.walk('./para/'):
    for i, filename in enumerate(files):
        if filename.endswith('.csv'):
            ...
            for i in range(len(df)):
                not_filt.append(len(df.loc[\
                            (df['own_mmsi'] == df['own_mmsi'][i]) &\
                            ((df['start_idx'] <= df['stop_idx'][i]) &\
                            (df['stop_idx'] >= df['start_idx'][i]) )]))
