...
def save_tracks(track_history, working_directory, reset=False):
    """
    Saves the tracks to a npy file
    """
    npy_file = f"{working_directory}/code/npy_files/tracks_2.npy"
    if not os.path.exists(npy_file) or reset:
        tracks_dict = {}
        counter = 0
    else:
        tracks_dict = np.load(npy_file,allow_pickle=True).item()
        counter = len(tracks_dict)

    for k,track in enumerate(track_history.items()):
        key = counter+k
        tracks_dict[key] = []
        for track_point in track[1]:
            mean, cov = track_point.posterior
            tracks_dict[key].append([mean[0],mean[2]])

    np.save(f"{working_directory}/code/npy_files/tracks.npy",tracks_dict)


def save_tracks_for_colreg(track_history, working_directory):
    """
    Saves the tracks to a npy file
    """
    npy_file = f"{working_directory}/code/npy_files/colreg_tracks.npy"
    tracks_dict = {}
    counter = 0
    for k,track in enumerate(track_history.items()):
        key = counter+k
        ...
        for track_point in track[1]:           
            ...
            x_vels.append(x_vel)
            y_vels.append(y_vel)
        tracks_dict[key] = [timestamps,xs,ys,yaws,x_vels,y_vels]
            # # print(f"Yaw = {np.degrees(yaw)}")
            # if prev_x_pos is not None:
            #     distance = np.sqrt((x_pos-prev_x_pos)**2 + (y_pos-prev_y_pos)**2)
            #     time = track_point.timestamp - prev_timestamp
            #     velocity = distance/time
            #     # print(f"Velocity calculated = {velocity}")
            #     # print(f"Velocity from track = {np.sqrt(x_vel**2 + y_vel**2)}")
            #     # # covaraince of the velocity from track
            #     # velocity_covariance = cov[1:3, 1:3]
            #     # print(f"Velocity covariance = {velocity_covariance}")
            # # temp_in = input("Press enter to continue")
            # prev_x_pos = x_pos
            # prev_y_pos = y_pos
            # prev_timestamp = timestamp
            # tracks_dict[key].append([timestamp,x_pos,y_pos, yaw, x_vel, y_vel])

        show_plot = False
        if show_plot:
            fig, ax = plt.subplots(figsize=(11, 7.166666))
            ax.plot(0,0,marker='o',color='red')
            x = [track_point[1] for track_point in tracks_dict[key]]
            y = [track_point[2] for track_point in tracks_dict[key]]
            yaws = [track_point[3] for track_point in tracks_dict[key]]
            ax.quiver(x[1:-1:3],y[1:-1:3],np.cos(yaws[1:-1:3]),np.sin(yaws[1:-1:3]), color='black', scale=2, scale_units='inches', width=0.001, zorder=10)
            ax.plot(x,y)
