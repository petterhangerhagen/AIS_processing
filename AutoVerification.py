...
class AutoVerification:
    ...
    def __init__(self,
        ...
        self.p_delta_v_down = np.zeros([self.n_vessels, self.n_vessels])  # Penalty: speed deincrease when stay-on vessel
    ...
    def entry_criteria(self, vessel, obst, i):
        ...
        if i == 0:
            try:
                nan_indx = np.append(np.array( [i for i,x in enumerate(vessel.speed) if np.isnan(x)] ), [0, len(vessel.speed)])
    ...
    def filterOutNonCompleteSituations(self, vessel, obst):
        ...
        if (self.CRGW or self.CRSO) in self.situation_matrix[vessel.id,obst.id, :]:
            ...
            if cdist > 100:
                area = (self.situation_matrix[vessel.id, obst.id, :] == self.CRGW) | (self.situation_matrix[vessel.id, obst.id, :] == self.CRSO)
                self.situation_matrix[vessel.id, obst.id, area] = self.NAR
    ...
    def constructParams(self, own_vessel, obst_vessel, start_idx, stop_idx):
        ...
        def getVesselParams(vessel, obst):
            if not vessel.maneuvers_found:
                ...
                if printer_on: print("\n\n")
                if printer_on: print(vessel.maneuver_start_stop)
                if printer_on: print("Start:", start_idx, " stop:", stop_idx)

                man_inside =((start_idx < vessel.maneuver_start_stop[:,0] ) & \
                        (stop_idx > vessel.maneuver_start_stop[:, 1] )) if len(vessel.maneuver_detect_idx) > 0 else np.array([], dtype=bool)
                man_inside = np.array(man_inside)
                if printer_on: print(man_inside)
                if printer_on: print(vessel.maneuver_detect_idx[man_inside])
                ...
                if len(vessel.maneuver_detect_idx[man_inside]) > 1:
                    ...
                    for inside in man_inside:
                        ...
                        if printer_on: print("Idx:", vessel.maneuver_detect_idx[i])
                        if printer_on: print("Man:", vessel.maneuver_start_stop[i])
                        
                        
                        range_val_start, time_to_cpa_start = calcPredictedCPA(vessel, obst, vessel.maneuver_start_stop[i][0])
                        if printer_on: print("R-start:", range_val_start)
                        range_val_stop, time_to_cpa_stop = calcPredictedCPA(vessel, obst, vessel.maneuver_start_stop[i][1])
                        if printer_on: print("R-stop:", range_val_stop)

                        if np.isnan(range_val_start) or \
                           np.isnan(range_val_stop):

                            continue

                        range_diff      = range_val_stop - range_val_start
                        if printer_on: print("R-diff:", range_diff)
                        
                        if maneuver_idx != None:
                            if range_diff < diff_man_dist:
                                continue

                        if printer_on: print("NEW BEST")
                    ...
                    for inside in man_inside:
                        if inside:
                            maneuver_idx = vessel.maneuver_detect_idx[i]
                            man_number = i
                            pre_man_dist, pre_man_t_cpa = calcPredictedCPA(vessel, obst, vessel.maneuver_start_stop[i][0])
                            post_man_dist, post_man_t_cpa = calcPredictedCPA(vessel, obst, vessel.maneuver_start_stop[i][1])
                ...
                if not pre_man_t_cpa is None:
                    pre_man_t_cpa -= maneuver_idx

                if not post_man_t_cpa is None:
        ...
        maneuver_made_own, man_number_own, maneuver_idx_own, multi_man_own, pre_man_dist_own, pre_man_t_cpa_own, post_man_dist_own, post_man_t_cpa_own = getVesselParams(own_vessel, obst_vessel)
        maneuver_made_obst, man_number_obst, maneuver_idx_obst, multi_man_obst, pre_man_dist_obst, pre_man_t_cpa_obst, post_man_dist_obst, post_man_t_cpa_obst = getVesselParams(obst_vessel, own_vessel)

        if maneuver_idx_own == None and maneuver_made_own:
            return None
    ...
    def getParameters(self, vessel, obst, start_idx, stop_idx,\
                      maneuver_made_own, man_number_own, maneuver_idx_own, multi_man_own, pre_man_dist_own, pre_man_t_cpa_own, post_man_dist_own, post_man_t_cpa_own,\
                      maneuver_made_obst, man_number_obst, maneuver_idx_obst, multi_man_obst, pre_man_dist_obst, pre_man_t_cpa_obst, post_man_dist_obst, post_man_t_cpa_obst):
        """
        Takes the timespan of a COLREG situation and returns ownship info, obstacle info, and parameters for
        any maneuvers made by ownship. Output is returned as a Parameters-object.
        :param vessel: ownship as Vessel-object
    ...
    def read_AIS(self, AIS_path, ship_path):
        ...
        for boat in boats:
                ...
                if boat.at[0, 'mmsi'] in ship_df.mmsi.tolist():
                    ...
                    if len(name_df) == 1:
                        ...
                        for date in name_df.index:
                            if  name_df.at[date, 'date_min'] <= boat.at[0, 'date_time_utc'] <= name_df.at[date, 'date_max']:
            ...
            vessel.travel_dist = np.linalg.norm([vessel.state[0, vessel.nan_idx[1]] - vessel.state[0, vessel.nan_idx[0]], \
            ...
            if vessel.travel_dist > 1000:
                ...
                target_area = [np.logical_and(np.logical_and(target_area[i] == True, target_area[i+2] == True), target_area[i+1] == True) for i in range(len(target_area)-2)]
                ...
                if speed.size >= 3:
                    ...
                    try:
                        vessel.speed_der[target_area] = [np.dot([speed[i], speed[i+1], speed[i+2]], [-0.5,0,0.5]) for i in range(len(speed)-2)]
                    except:
                        pass 
                ...
                a[target_area] = a[target_area] - s[target_area] # this is to not have sudden changes from pi to -pi or opposite count as maneuvers
                from scipy.ndimage.filters import gaussian_filter
                a[target_area] = gaussian_filter(a[target_area], sigma=2)


                target_area = [np.logical_and(target_area[i] == True, True == target_area[i+2]) for i in range(len(target_area)-2)]
                target_area = np.append(False, target_area)
                target_area = np.append(target_area, False)
                if a.size >= 3:
                    a = a[np.isnan(a) == False]
                    vessel.maneuver_der[0, :] = [0 for i in range(len(vessel.state[2,:]))]
                    vessel.maneuver_der[0, target_area] = [np.dot([a[i], a[i+1], a[i+2]], [-0.5,0,0.5]) for i in range(len(a)-2)]
                    vessel.maneuver_der[1, :] = [0 for i in range(len(vessel.state[2,:]))]
                    vessel.maneuver_der[1, target_area] = [np.dot([a[i], a[i+1], a[i+2]], [1,-2,1]) for i in range(len(a)-2)]

                    target_area = [np.logical_and(target_area[i] == True, True == target_area[i+2]) for i in range(len(target_area)-2)]
                    target_area = np.append(False, target_area)
                    target_area = np.append(target_area, False)
                    vessel.maneuver_der[2, :] = [0 for i in range(len(vessel.state[2,:]))]
                    vessel.maneuver_der[2, target_area] = [np.dot([a[i], a[i+1], a[i+2], a[i+3], a[i+4]], [-0.5,1,0,-1,0.5]) for i in range(len(a) - 4)]
                    vessel.maneuver_der[1, :] = [0 for i in range(len(vessel.state[2,:]))]
                    vessel.maneuver_der[1, target_area] = [np.dot([a[i], a[i+1], a[i+2], a[i+3], a[i+4]], [-1/12,4/3,-5/2,4/3,-1/12]) for i in range(len(a) - 4)]
    ...
    def plot_trajectories2(self, show_trajectory_at = None, specific_obst_names = None, save = False, save_folder = 'img'):
        ...
        for vessel in self.vessels: # Plot trajectories for all vessels
            ...
            x, y = m(vessel.stateLonLat[0, list(vessel.maneuver_detect_idx)], vessel.stateLonLat[1, list(vessel.maneuver_detect_idx)])
            ...
            if vessel.id != self.OWN_SHIP:  # Mark CPA and plot situations
                ...
                x_cpa, y_cpa = m(self.vessels[self.OWN_SHIP].stateLonLat[0, cpa_idx], self.vessels[self.OWN_SHIP].stateLonLat[1, cpa_idx])
                p3 = ax1.scatter(x_cpa, y_cpa, marker='D', color=c)

                ax1.scatter(x_cpa, y_cpa, marker='D', color='k')
                

                x_man, y_man = m(vessel.stateLonLat[0, list(vessel.maneuver_detect_idx)], vessel.stateLonLat[1, list(vessel.maneuver_detect_idx)])
        ...
        try:
            ax1.legend([own_line, obs_line, (p1, p2), (p3, p4)], [own_label, obst_label, 'Maneuver', 'CPA'], scatterpoints=1,
        ...
        if save:
            save_name = "./" + save_folder + "/" + self.vessels[self.OWN_SHIP].name + "-" + specific_obst_names[0] + "-" + self.case_name + ".png"
    ...
    def plot_trajec_col(self, specific_obst_names = None):
        ...
        for vessel, color in zip(self.vessels, colors): # Plot trajectories for all vessels
            ...
            if vessel.id != self.OWN_SHIP and not any(x != 0 for x in self.situation_matrix[self.OWN_SHIP, vessel.id, :]):
            ...
            if vessel.id == self.OWN_SHIP:
                ax1.scatter(vessel.stateLonLat[0, list(vessel.maneuver_detect_idx)], vessel.stateLonLat[1, list(vessel.maneuver_detect_idx)],
                            marker='o',label = 'maneuver', color=color)
            else:
                ax1.scatter(vessel.stateLonLat[0, list(vessel.maneuver_detect_idx)], vessel.stateLonLat[1, list(vessel.maneuver_detect_idx)],
                            marker='o', color=color)
            ...
            if vessel.id != self.OWN_SHIP:  # Mark CPA and plot situations
                ...
                ax2.plot(self.situation_matrix[self.OWN_SHIP, vessel.id],
                         label=(str(self.vessels[self.OWN_SHIP].name) + ' wrt. ' + str(label)) if label != '' else '', color=color)
    ...
    def plot_trajectories(self, show_trajectory_at = None, specific_obst_names = None):
        ...
        if True:
            ...
            for i, m in enumerate(self.vessels[self.OWN_SHIP].maneuver_detect_idx):
                try:
                    fulltext += '\n %i AT=%i, c:%.2f, s:%.1f'%(i, m, self.vessels[self.OWN_SHIP].delta_course[i], self.vessels[self.OWN_SHIP].delta_speed[i])
                except:
